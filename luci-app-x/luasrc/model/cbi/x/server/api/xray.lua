module("luci.model.cbi.x.server.api.xray", package.seeall)
...
function gen_config(user)
    ...
    if user.transit_node and user.transit_node ~= "nil" then
        local gen_xray = require("luci.model.cbi.x.api.gen_xray")
        local client = gen_xray.gen_outbound(ucic:get_all("x", user.transit_node), "transit")
        table.insert(outbounds, 1, client)
    end

    local config = {
        log = {
            -- error = "/var/etc/x_server/log/" .. user[".name"] .. ".log",
