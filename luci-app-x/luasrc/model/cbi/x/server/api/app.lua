    ...
    ucic:foreach(CONFIG, "user", function(user)
        ...
        if enable and tonumber(enable) == 1 then
            ...
            elseif type == "SS" or type == "SSR" then
                config = require("luci.model.cbi.x.server.api.shadowsocks").gen_config(user)
                local udp_param = ""
            ...
            elseif type == "Xray" then
                config = require("luci.model.cbi.x.server.api.xray").gen_config(user)
                bin = ln_start(api.get_xray_path(), "xray", "-config=" .. config_file, log_path)
            elseif type == "Trojan" then
                config = require("luci.model.cbi.x.server.api.trojan").gen_config(user)
