...
module("luci.controller.x", package.seeall)
...
function index()
	appname = require "luci.model.cbi.x.api.api".appname
	entry({"admin", "services", appname}).dependent = true
	entry({"admin", "services", appname, "reset_config"}, call("reset_config")).leaf = true
	entry({"admin", "services", appname, "show"}, call("show_menu")).leaf = true
	entry({"admin", "services", appname, "hide"}, call("hide_menu")).leaf = true
	if not nixio.fs.access("/etc/config/x") then return end
	if nixio.fs.access("/etc/config/x_show") then
		entry({"admin", "services", appname}, alias("admin", "services", appname, "settings"), _("X"), 1).dependent = true
	end
	--[[ Client ]]
...
function reset_config()
	luci.sys.call('[ -f "/usr/share/x/config.default" ] && cp -f /usr/share/x/config.default /etc/config/x && /etc/init.d/x reload')
	luci.http.redirect(api.url())
end

function show_menu()
	luci.sys.call("touch /etc/config/x_show")
	luci.http.redirect(api.url())
end

function hide_menu()
	luci.sys.call("rm -rf /etc/config/x_show")
...
function link_add_node()
	...
	luci.sys.call("lua /usr/share/x/subscribe.lua add log")
end

function get_now_use_node()
	local e = {}
	local data, code, msg = nixio.fs.readfile("/var/etc/x/id/TCP")
	if data then
		e["TCP"] = util.trim(data)
	end
	local data, code, msg = nixio.fs.readfile("/var/etc/x/id/UDP")
	if data then
...
function get_redir_log()
	...
	if nixio.fs.access("/var/etc/x/" .. filename .. ".log") then
		local content = luci.sys.exec("cat /var/etc/x/" .. filename .. ".log")
...
function get_log()
	-- luci.sys.exec("[ -f /var/log/x.log ] && sed '1!G;h;$!d' /var/log/x.log > /var/log/x_show.log")
	luci.http.write(luci.sys.exec("[ -f '/var/log/x.log' ] && cat /var/log/x.log"))
end

function clear_log()
	luci.sys.call("echo '' > /var/log/x.log")
end
...
function status()
	...
	e["tcp_node_status"] = luci.sys.call(string.format("top -bn1 | grep -v -E 'grep' | grep '%s/bin/' | grep -i 'TCP' >/dev/null", appname)) == 0
...
function set_node()
	...
	ucic:commit(appname)
	luci.sys.call("/etc/init.d/x restart > /dev/null 2>&1 &")
...
function update_rules()
	...
	luci.sys.call("lua /usr/share/x/rule_update.lua log '" .. update .. "' > /dev/null 2>&1 &")
end
...
function server_user_log()
	local id = luci.http.formvalue("id")
	if nixio.fs.access("/var/etc/x_server/" .. id .. ".log") then
		local content = luci.sys.exec("cat /var/etc/x_server/" .. id .. ".log")
		content = content:gsub("\n", "<br />")
		luci.http.write(content)
	else
		luci.http.write(string.format("<script>alert('%s');window.close();</script>", i18n.translate("Not enabled log")))
...
function server_get_log()
	luci.http.write(luci.sys.exec("[ -f '/var/log/x_server.log' ] && cat /var/log/x_server.log"))
end

function server_clear_log()
	luci.sys.call("echo '' > /var/log/x_server.log")
end
